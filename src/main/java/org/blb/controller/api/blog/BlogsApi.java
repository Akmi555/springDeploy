...
public interface BlogsApi {
    ...
    @Operation(summary = "Adding blog views", description = "The operation is available to everyone")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "info abut success adding.",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = ContentResponseDTO.class))),
            @ApiResponse(responseCode = "404", description = "Blog with ID  .. not found",
                    content = @Content(mediaType = "application/json",
    ...
    @PostMapping()
    ResponseEntity<StandardResponseDto> addBlogViews(@Valid @RequestBody  IdRequestDTO dto);
}
